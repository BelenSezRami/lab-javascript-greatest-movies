...
function howManyMovies(moviesArray) {
    ...
    const filteredMovies = moviesArray.filter((eachMovie) => {
        ...
        if (eachMovie.genre.includes("Drama") && eachMovie.director.toLowerCase() === 'steven spielberg') {
...
function orderByYear(moviesArray) {
    ...
    // console.log(newMoviesArray.length)


    const orderedArray = moviesArrayCopy.sort((a, b) => {
...
function orderAlphabetically(moviesArray) {
    ...
    const moviesArrayCopy2 = JSON.parse(JSON.stringify(moviesArray))


    //console.log(moviesArrayCopy2)


    const orderedArray2 = moviesArrayCopy2.sort((a, b) => {
    ...
    const titlesArray = orderedArray2.map((eachMovie) => {
